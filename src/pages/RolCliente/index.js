...
function ClienteHome({ data }) {
  ...
  const debounceProdFilter = debounce((text) => {
    ...
    let cardF = data.filter(x => x.nombre.toLowerCase().includes(text.toLowerCase()))
        ...
        <section style={{ position: 'relative' }} className='section_Pagin'>
          ...
          </section>
          {
            openListV ? cardFilter.length != 0 ?
              ...
              (<div style={{ display: 'flex', flexWrap: 'wrap', gap: '13px', justifyContent: 'center' }}>
                {
                  [...data].map((x, index) => (<CardUserNew item={x} index={index} key={x.id} clickIcon={setLoading} />))
                }
              </div>)



              : cardFilter.length != 0 ? <CarruselVendedores clickIcon={setLoading} items={cardFilter} /> : <CarruselVendedores clickIcon={setLoading} items={[...data]} />
          }
