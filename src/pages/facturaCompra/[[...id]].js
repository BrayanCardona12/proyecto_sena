...
FacturaComp.getInitialProps = async (ctx) => {
    try {
        const response3 = await axios.delete(host + '/api/compra/?idP=' + parseInt(ctx.query.id[2]));
        const response2 = await axios.get(host + `/api/Login?id=${parseInt(ctx.query.id[1])}`);
        const response1 = await axios.get(host + `/api/Login?id=${parseInt(ctx.query.id[0])}`);
